...
/**
 * @brief Construct a new SyulimeTracker_BNO08x::SyulimeTracker_BNO08x object
 *
 * @param sda The arduino pin # connected to the I2C SDA line
 * @param scl The arduino pin # connected to the I2C SCL line
 * @param reset_pin The arduino pin # connected to the BNO Reset pin
 */
SyulimeTracker_BNO08x::SyulimeTracker_BNO08x(int sda, int scl, int8_t reset_pin)
{
  _sda = sda;
  _scl = scl;
...
/**
 * @brief Destroy the SyulimeTracker_BNO08x::SyulimeTracker_BNO08x object
 *
 */
SyulimeTracker_BNO08x::~SyulimeTracker_BNO08x(void)
{
  // if (temp_sensor)
  //   delete temp_sensor;
}
 ...
 */
bool SyulimeTracker_BNO08x::begin_I2C(uint8_t i2c_address, TwoWire *wire,
                                      int32_t sensor_id)
{
  if (i2c_dev)
  {
  ...
  // コンストラクタで指定されたSDA/SCLピンを使用してI2Cデバイスを作成
  i2c_dev = new SyulimeTracker_I2CDevice(i2c_address, _sda, _scl, wire);
 ...
 * @return  true if initialization was successful, otherwise false.
 */
bool SyulimeTracker_BNO08x::begin_UART(HardwareSerial *serial, int32_t sensor_id)
{
  uart_dev = serial;
 ...
 *    @return true if initialization was successful, otherwise false.
 */
bool SyulimeTracker_BNO08x::begin_SPI(uint8_t cs_pin, uint8_t int_pin,
 ...
 */
bool SyulimeTracker_BNO08x::_init(int32_t sensor_id)
{
  int status;
 ...
 *
 */
void SyulimeTracker_BNO08x::hardwareReset(void) { hal_hardwareReset(); }
 ...
 */
bool SyulimeTracker_BNO08x::wasReset(void)
{
 ...
 */
bool SyulimeTracker_BNO08x::getSensorEvent(sh2_SensorValue_t *value)
{
  _sensor_value = value;
 ...
 */
bool SyulimeTracker_BNO08x::enableReport(sh2_SensorId_t sensorId,
